  ...
  router.use(function (err, req, res, next) {
    ...
    res
      ...
      .json(
        utils.errors.get(
          500,
          'The server encountered an unexpected condition: ' + err.message
        )
      );
  });
} else {
...
router.get(
  '/agencies/:agency/routes/:route/stops/:stop/predictions',
  api.predictions.get
);
  ...
  if (method.toLowerCase() !== 'get') {
    res
      ...
      .json(
        utils.errors.get(
          405,
          'HTTP method ' + method + ' is not supported by this URI resource.'
        )
      );
  } else {
    res
      .status(404)
      .json(utils.errors.get(404, 'The requested URI can not be found on this server.'));
  }
});
...
restbus.close = function (callback) {
  ...
  if (!demo)
    restbus.close = function () {
      console.log('Can only call restbus.close() once!');
    };
