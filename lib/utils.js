    ...
    try {
      js4xml(xml, function (err, js) {
        if (err) callback(err, { errtype: 'parse' });
          ...
          callback(
            { message: 'No "body" element in NextBus XML document' },
            { errtype: 'parse' }
          );
...
utils.nbRequestError = function (e, res) {
  ...
  res
    .status(500)
    .json(
      this.errors.get(500, 'Unable to fulfill request to ' + host + '. ' + e.message)
    );
};
...
utils.streamOrParseError = function (e, js, res) {
  if (js.errtype === 'parse') {
    res
      .status(500)
      .json(this.errors.get(500, 'Unable to parse XML into JavaScript. ' + e.message));
  } else {
    res
      .status(500)
      .json(this.errors.get(500, 'An unexpected stream error has occured. ' + e.message));
  }
};
