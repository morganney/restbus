  ...
  http.get(utils.getOptionsWithPath(path), function(nbres) {
    utils.getJsFromXml(nbres, function(err, js) {
      ...
      if(!err) {
        ...
        uri = utils.getClosedPath(req).split(onsplit)[0]; // closed/open irrelevant.
        if(!nberr) {
          ...
          js.body.predictions.forEach(function(pred) {
            ...
            if(!$.dirTitleBecauseNoPredictions) {
              ...
              selflink = {
                ...
                transition: 'Current representation of this prediction resource.',
                resource: 'prediction',
                type: 'collection'
              };
              ...
              p._links = {
                self: selflink,
                to: [],
                from: onsplit === 'routes' ? [] : [selflink] // both tuples/codes link-from canonical prediction.
              };
              json.push(p);
