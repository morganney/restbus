...
function getLinks(hrefs) {
  var predictions = [],
      to = [{
        method: 'GET',
        href: hrefs.vehicle,
        transition: 'Collection of vehicle resources for the route.',
        resource: 'vehicle',
        type: 'collection'
      }];
  ...
  hrefs.stops.forEach(function(stop) {
    ...
    p.transition = 'Collection of prediction resources for the current route, transit agency and stop id ' + stop.id;
    p.resource = 'prediction';
    p.type = 'collection';
  ...
  return {
    self: {
      method: 'GET',
      href: hrefs.self,
      transition: 'Current respresentation for route resource.',
      resource: 'route',
      type: 'unit'
    },
    to: to,
          ...
          js.body.route.forEach(function(route) {
            ...
            r._links = {
              self: {
                method: 'GET',
                href: suri,
                transition: 'Current and more complete respresentation for route resource.',
                resource: 'route',
                type: 'unit'
              },
              to: [
                {
                  method: 'GET',
                  href: suri,
                  transition: 'Current and more complete representation for route resource.',
                  resource: 'route',
                  type: 'unit'
                }
              ],
...
routes.get = function(req, res) {
  ...
  http.get(utils.getOptionsWithPath(path), function(nbres) {
    utils.getJsFromXml(nbres, function(err, js) {
      var json = {},
          route, $, nberr, uri, vuri, puri, ruri;
      ...
      if(!err) {
        ...
        if(!nberr) {
          ...
          json._links = getLinks({self: uri, vehicle: vuri, stops: json.stops, route: uri.replace('/' + json.id, '')});
          res.json(200, json);
        } else utils.nbXmlError(nberr, res);
      } else utils.streamOrParseError(err, js, res);
    });
