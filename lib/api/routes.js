...
function getLinks(hrefs) {
  var to = [{resource: 'vehicle', href: hrefs.vehicle, type: 'collection'}],
      predictions = [];
  ...
  return {
    ...
    to: to,
    from: [{resource: 'route', href: hrefs.route, type: 'collection'}]
  };
}
        ...
        if(!nberr) {
          js.body.route.forEach(function(route) {
            ...
            r._links = {
              ...
              to: [
                {resource: 'route', href: suri, type: 'unit'}
              ],
              from: [
                {resource: 'route', href: uri, type: 'collection'}
              ]
            };
            json.push(r);
          });
